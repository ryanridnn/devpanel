	...
	<Modal title="Update" :opened="opened" :close="close">
		<div class="form">
			<div class="form__avatar">
				...
				<label for="update__avatar" class="form__avatar__label"
					>Select Avatar</label
				>
				<input
					@change="changeAvatar"
			...
			<div class="form__deployed">
				<div class="form__deployed__label-group">
					<label for="update__deployed">Deployed</label>
					<label
						for="update__deployedToggle"
...
export default {
	...
	methods: {
		...
		async handleSubmit() {
			...
			if (
				...
				this.path.source === this.project.path.source &&
				this.path.url === this.project.path.url &&
				this.deployed.isDeployed === this.project.deployed.isDeployed
			) {
				if (
					this.deployed.isDeployed &&
					this.deployed.url !== this.project.deployed.url
				) {
				} else if (
					this.tags.length === this.project.tags.length &&
					this.tags.every((tag) =>
						this.project.tags.some((t) => t._id === tag._id)
					)
				) {
					return;
				}
			...
			project.append("_id", this.project._id);
	...
	watch: {
		"deployed.isDeployed"() {
			if (this.deployed && !this.deployed.isDeployed)
				this.deployed.url = null;
		},
		project() {
			const {
				_id,
			...
			this.deployed = { ...deployed } || { isDeployed: false, url: null };
			this.tags = tags ? [...tags] : [];
		},
	},
};
